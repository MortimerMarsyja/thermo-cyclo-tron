...
const HeatingView = ({ isDarkMode = false, themeColors, navigate }: HeatingViewProps) => {
  ...
  const currentStage = config.stages[currentStageIndex] || { stage: "Default", temp: 95, time: 120 };
  ...
  }, [isRunning, isComplete, currentStageIndex, currentStage.time, config.stages]);

  // Calculate progress percentage
  const progress = ((totalElapsed + (currentStage.time - timeRemaining)) / config.totalTime) * 100;
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ backgroundColor: theme.cardBackground, border: `2px solid ${theme.border}` }}
      >
        {isComplete ? (
          <div>
            ...
            <p class="text-lg mb-6" style={{ color: theme.textSecondary }}>
              Thermal cycling has finished successfully with {config.stages.length} stages.
            </p>
          ...
          <div>
            <div class="flex items-center justify-center mb-4">
              ...
              <h2 class="text-2xl font-bold" style={{ color: theme.text }}>
                {currentStage.stage} - Step {currentStageIndex + 1} of {config.stages.length}
              </h2>
            </div>
            
            <div class="grid grid-cols-2 gap-6 mb-6">
              <div class="text-center">
                <p class="text-sm" style={{ color: theme.textSecondary }}>Current Temperature</p>
                <p class="text-3xl font-bold" style={{ color: "#dc2626" }}>
                  {currentStage.temp}°C
                </p>
              </div>
              <div class="text-center">
                <p class="text-sm" style={{ color: theme.textSecondary }}>Time Remaining</p>
            ...
            <div class="mb-6">
              <div class="flex justify-between text-sm mb-2" style={{ color: theme.textSecondary }}>
                <span>Overall Progress</span>
                <span>{Math.round(progress)}%</span>
              </div>
              <div class="w-full h-4 rounded-full overflow-hidden" style={{ backgroundColor: theme.border }}>
                <div 
              ...
              <p class="text-xs mt-2 text-center" style={{ color: theme.textSecondary }}>
                Total Time: {formatTime(config.totalTime)} | Stages: {config.stages.length}
              </p>
              {config.isConfigured && (
                <p class="text-xs mt-1 text-center" style={{ color: "#16a34a" }}>
